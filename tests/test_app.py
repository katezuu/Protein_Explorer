...
def test_post_invalid_pdb(client):
    resp = client.post('/', data={'pdb_id1': 'ABC', 'pdb_id2': ''}, follow_redirects=True)
    assert resp.status_code == 200
    assert 'Please enter a valid 4-character PDB ID #1' in resp.get_data(as_text=True)
...
def test_post_valid_but_nonexistent_pdb(monkeypatch, client):
    ...
    resp = client.post('/', data={'pdb_id1': 'ZZZZ', 'pdb_id2': ''}, follow_redirects=True)
    assert resp.status_code == 200
